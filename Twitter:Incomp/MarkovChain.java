...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        ...
        ProbabilityDistribution<String> prob1 = new ProbabilityDistribution<>();
        prob1.record(second);
        if (this.chain.containsKey(first)) {
            ProbabilityDistribution<String> existing = this.chain.get(first);
            existing.record(second);
        } else {
            this.chain.put(first, prob1);
        }
    }
    ...
    public void reset(String start) {
        ...
        try {
            ...
            throw new NoSuchElementException("start not present in training data");
        }
    }
    ...
    public String next() {
        ...
        if (chain.containsKey(current)) {
            ProbabilityDistribution<String> pd = this.chain.get(this.current);
            this.current = pd.pick(this.ng);
        } else {
            current = END_TOKEN;
        }
        return s;
