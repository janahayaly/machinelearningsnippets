...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        try {
            ...
            String col = splitString[csvColumn];
            if (col.isEmpty()) {
                return null;
            } else {
                return col;
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (iterator.hasNext()) {
            String tweet = extractColumn(iterator.next(), tweetColumn);
            if (tweet != null) {
                sentences.add(tweet);
            }
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        for (String sentence : sentences) {
            List<String> words = parseAndCleanSentence(sentence);
            result.add(words);
        }
        return result;
    }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        List<List<String>> result = new LinkedList<>();
        List<String> sentences = csvDataToTweets(br, tweetColumn);
        for (String sentence : sentences) {
            List<List<String>> words = parseAndCleanTweet(sentence);
            result.addAll(words);
        }
        return result;
    }
