...
public class TwitterBot {
    ...
    public TwitterBot(BufferedReader br, int tweetColumn, NumberGenerator ng) {
        ...
        this.ng = ng;
        List<List<String>> trainedData = TweetParser.csvDataToTrainingData(br, tweetColumn);
        for (List<String> sentence : trainedData) {
    ...
    public String generateTweet(int numWords) {
        ...
        String tweet = "";
        if (numWords < 0) {
            throw new IllegalArgumentException("Number of words is negative");
        }
        int i = 0;
        while (i < numWords && this.mc.hasNext()) {
            while (i < numWords && this.mc.hasNext()) {
                tweet += " " + this.mc.next();
                i++;
            }
